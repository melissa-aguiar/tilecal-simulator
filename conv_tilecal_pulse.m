...
% 27 da função
function [sinal1,S_truth1]=conv_tilecal_pulse(S_truth,desvio,posicao1)
load('pulsos_fases.mat')
matriz_sorteio = matrix;

%% sinal da distribuição "CONVOLUÇÃO"
[N_A,c] = size(matriz_sorteio);
fase_posicao = randi([1,c],1,length(posicao1));                 %Realizo o sorteio do desvio de fase de -5 a +5 ns
centro = .5 + c/2;
a = ((N_A-1)/2);
